...
def signal_handler(signal_generator, queue):
    ...
    for signal in signal_generator:
        if signal:  # 确保信号不为空
            ...
            if signal.strip().lower() in ('catch_ok', 'release_ok'):
                temp = 1
                queue.put_nowait(('motion', signal))
            elif signal.strip().lower() == 'capture':
                temp = 2
                queue.put_nowait(('camera', signal))
            print(queue.qsize())
            time.sleep(0.1)
...
def control_robot_motion(robot, clientID, queue):
    ...
    while True:
        ...
        try:
            if temp == 1: 
                msg_type, signal = queue.get_nowait()  
                if msg_type == 'motion':  # 只处理运动控制信号
...
def main():
    ...
    # 创建TCP信号生成器
    signal_generator = tcp_server('169.254.15.159', 8888)  # 使用您想要的端口
    ...
    try:
        ...
        while True:
            ...
            # 控制更新频率在50Hz左右
            if current_time - last_time >= 0.01: #0.02
                # 只处理最新数据
                joint_angles = robot.get_joint_angles()
                if joint_angles is not None:
                    set_joint_angles(clientID, '/1axis', joint_angles)
